...
IF OBJECT_ID('dbo.usp_GetMail_4_SQLDBATools') IS NULL
	EXEC ('CREATE PROCEDURE [dbo].[usp_GetMail_4_SQLDBATools] AS RETURN 0;');
GO
...
BEGIN
	...
	*/
	...
	BEGIN
		SELECT @tableName = 'dbo.vw_DatabaseBackups';
	...
	SET @subject = @p_Option + ' - '+CAST(CAST(GETDATE() AS DATE) AS VARCHAR(20));
	IF @p_Verbose = 1
