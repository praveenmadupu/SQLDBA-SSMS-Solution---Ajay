...
BEGIN TRY
	...
	SELECT * 
	FROM	(VALUES	('environment','Prod, Stage, QA, Dev, Test')
					,('skip backup','Yes,No')
					,('DefaultFullBackupDestination','Local backup directory like F:\Backups\')
			) AS pt ([p_key], [p_valueExplanation])
END TRY
BEGIN CATCH
...
BEGIN TRY
	...
	([p_key], [p_value])
	SELECT * 
	FROM	(VALUES	('environment','Prod')
					,('DefaultFullBackupDestination','F:\Backups\')
			) AS pt ([p_key], [p_value])
END TRY
BEGIN CATCH
