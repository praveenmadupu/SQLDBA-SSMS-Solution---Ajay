...
select	CollectionBatchNO, collection_time, TimeInMinutes, [dd hh:mm:ss.mss], [dd hh:mm:ss.mss (avg)], session_id, sql_text, sql_command,
		query_plan, 
		login_name, wait_info, tasks, tran_log_writes, CPU, tempdb_allocations, tempdb_current, blocking_session_id, blocked_session_count, reads, writes, context_switches, physical_io, physical_reads, locks, used_memory, status, tran_start_time, open_tran_count, percent_complete, host_name, database_name, program_name, additional_info, start_time, login_time, request_id 
from tQueries
