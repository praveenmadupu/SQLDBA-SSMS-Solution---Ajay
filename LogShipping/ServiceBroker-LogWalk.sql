...
-- Step 09:		Add a step for Job in [DBA Log Walk Alerts] job
declare @c_jobName varchar(255);
declare @c_jobStepText varchar(max);

declare cur_jobs cursor forward_only local read_only for 
		SELECT j.name
...
OPEN cur_jobs;
FETCH NEXT FROM cur_jobs into @c_jobName, @c_jobStepText;
