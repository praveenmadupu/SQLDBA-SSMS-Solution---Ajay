...
/*	1) https://github.com/olahallengren/sql-server-maintenance-solution/issues/14
	2) https://github.com/BrentOzarULTD/sql-server-maintenance-solution
*/
...
BEGIN
  ...
  DECLARE @DirectoryOverride nvarchar(4000)
  DECLARE @CurrentDirectoryOverride nvarchar(4000)
  DECLARE @MirrorDirectoryOverride nvarchar(4000)
  DECLARE @CurrentMirrorDirectoryOverride nvarchar(4000)
  ...
  BEGIN
    /* They specified string replacements. Save those off and handle them later. */
    SET @DirectoryOverride = SUBSTRING(@Directory, CHARINDEX('**',@Directory), (LEN(@Directory) - CHARINDEX('**',@Directory)) + 1);
    SET @Directory = SUBSTRING(@Directory, 1, (CHARINDEX('**',@Directory) - 1));
  END
  IF CHARINDEX('*', @Directory) > 0
  BEGIN
    /* They specified string replacements. Save those off and handle them later. */
    SET @DirectoryOverride = '';
    SET @Directory = REPLACE(@Directory, '*', ''); -- replace * with blank
  END 
  IF CHARINDEX('**', @MirrorDirectory) > 0
  BEGIN
    SET @MirrorDirectoryOverride = SUBSTRING(@MirrorDirectory, CHARINDEX('**',@MirrorDirectory), (LEN(@MirrorDirectory) - CHARINDEX('**',@MirrorDirectory)) + 1);
    SET @MirrorDirectory = SUBSTRING(@MirrorDirectory, 1, (CHARINDEX('**',@MirrorDirectory) - 1));
  END 
  IF CHARINDEX('*', @MirrorDirectory) > 0
  BEGIN
    /* They specified string replacements. Save those off and handle them later. */
    SET @MirrorDirectoryOverride = '';
    SET @MirrorDirectory = REPLACE(@MirrorDirectory, '*', ''); -- replace * with blank
  END 
    ...
    */
    ...
    BEGIN
      ...
      BEGIN
        ...
        ELSE /* No instance installed */
        BEGIN
            SET @CurrentMirrorDirectoryOverride = REPLACE(@CurrentMirrorDirectoryOverride, '**SERVERNAMEWITHOUTINSTANCE**', CAST(SERVERPROPERTY('servername') AS nvarchar(max)));
            SET @CurrentMirrorDirectoryOverride = REPLACE(@CurrentMirrorDirectoryOverride, '**INSTANCENAME**', 'DEFAULT');
        END
        SET @CurrentMirrorDirectoryOverride = REPLACE(@CurrentMirrorDirectoryOverride, '**DATABASENAME**', @CurrentDatabaseNameFS);
        SET @CurrentMirrorDirectoryOverride = REPLACE(@CurrentMirrorDirectoryOverride, '**BACKUPTYPE**', UPPER(@CurrentBackupType));
      END /* IF @MirrorDirectoryOverride IS NOT NULL */
      ...
      BEGIN
        ...
        BEGIN
          ...
          SET @CurrentFilePath = @CurrentDirectoryPath + (CASE WHEN @DirectoryOverride IS NOT NULL AND LEN(LTRIM(RTRIM(@DirectoryOverride))) = 0 THEN '' ELSE '\' END) + CASE WHEN @CurrentAvailabilityGroup IS NOT NULL THEN @Cluster + '$' + @CurrentAvailabilityGroup ELSE REPLACE(CAST(SERVERPROPERTY('servername') AS nvarchar(max)),'\','$') END + '_' + @CurrentDatabaseNameFS + '_' + UPPER(@CurrentBackupType) + CASE WHEN @ReadWriteFileGroups = 'Y' THEN '_PARTIAL' ELSE '' END + CASE WHEN @CopyOnly = 'Y' THEN '_COPY_ONLY' ELSE '' END + '_' + REPLACE(REPLACE(REPLACE((CONVERT(nvarchar,@CurrentDate,120)),'-',''),' ','_'),':','') + CASE WHEN @NumberOfFiles > 1 AND @NumberOfFiles <= 9 THEN '_' + CAST(@CurrentFileNumber AS nvarchar) WHEN @NumberOfFiles >= 10 THEN '_' + RIGHT('0' + CAST(@CurrentFileNumber AS nvarchar),2) ELSE '' END + '.' + @CurrentFileExtension

          IF LEN(@CurrentFilePath) > 259
          BEGIN
            SET @CurrentFilePath = @CurrentDirectoryPath + (CASE WHEN @DirectoryOverride IS NOT NULL AND LEN(LTRIM(RTRIM(@DirectoryOverride))) = 0 THEN '' ELSE '\' END) + @CurrentDatabaseNameFS + '_' + UPPER(@CurrentBackupType) + CASE WHEN @ReadWriteFileGroups = 'Y' THEN '_PARTIAL' ELSE '' END + CASE WHEN @CopyOnly = 'Y' THEN '_COPY_ONLY' ELSE '' END + '_' + REPLACE(REPLACE(REPLACE((CONVERT(nvarchar,@CurrentDate,120)),'-',''),' ','_'),':','') + CASE WHEN @NumberOfFiles > 1 AND @NumberOfFiles <= 9 THEN '_' + CAST(@CurrentFileNumber AS nvarchar) WHEN @NumberOfFiles >= 10 THEN '_' + RIGHT('0' + CAST(@CurrentFileNumber AS nvarchar),2) ELSE '' END + '.' + @CurrentFileExtension
          END

          IF LEN(@CurrentFilePath) > 259
          BEGIN
            SET @CurrentFilePath = @CurrentDirectoryPath + (CASE WHEN @DirectoryOverride IS NOT NULL AND LEN(LTRIM(RTRIM(@DirectoryOverride))) = 0 THEN '' ELSE '\' END) + LEFT(@CurrentDatabaseNameFS,CASE WHEN (LEN(@CurrentDatabaseNameFS) + 259 - LEN(@CurrentFilePath) - 3) < 20 THEN 20 ELSE (LEN(@CurrentDatabaseNameFS) + 259 - LEN(@CurrentFilePath) - 3) END) + '...' + '_' + UPPER(@CurrentBackupType) + CASE WHEN @ReadWriteFileGroups = 'Y' THEN '_PARTIAL' ELSE '' END + CASE WHEN @CopyOnly = 'Y' THEN '_COPY_ONLY' ELSE '' END + '_' + REPLACE(REPLACE(REPLACE((CONVERT(nvarchar,@CurrentDate,120)),'-',''),' ','_'),':','') + CASE WHEN @NumberOfFiles > 1 AND @NumberOfFiles <= 9 THEN '_' + CAST(@CurrentFileNumber AS nvarchar) WHEN @NumberOfFiles >= 10 THEN '_' + RIGHT('0' + CAST(@CurrentFileNumber AS nvarchar),2) ELSE '' END + '.' + @CurrentFileExtension
          END
