...
print 'create #SysProcesses table with blocking details'
select  Concat
        (
...
where r.blocked <> 0 or r.spid in (select p.blocked from sys.sysprocesses p where p.blocked <> 0)
...
-- drop table audit_archive.dbo.blocking_report
